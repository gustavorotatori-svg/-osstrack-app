...
export function DailyMissions() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/missoes").then((r) => r.json()),
      fetch("/api/premium").then((r) => r.json()),
    ])
      .then(([missoesData, premiumData]) => {
      ...
      <div className="bg-gradient-to-br from-[var(--dark-card)] to-black/40 border border-[var(--dark-border)] rounded-2xl p-5 hover-card">
        ...
        {!isPremium && (
          <button
            ...
            className="w-full mt-3 py-2.5 rounded-xl bg-[rgba(201,168,76,0.1)] border border-[rgba(201,168,76,0.2)] text-xs text-[var(--gold)] font-semibold"
          >
