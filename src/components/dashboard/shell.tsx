      ...
      {showMobileMenu && (
        <div className="fixed inset-0 z-40 md:hidden" onClick={() => setShowMobileMenu(false)}>
          ...
          <div className="absolute right-3 top-16 w-56 surface py-2 overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={toggleTheme} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--bg-input)] transition-colors">
              {theme === "dark" ? <SunIcon className="w-4 h-4" /> : <MoonIcon className="w-4 h-4" />} <span>{t(theme === "dark" ? "modoClaro" : "modoEscuro")}</span>
            </button>
