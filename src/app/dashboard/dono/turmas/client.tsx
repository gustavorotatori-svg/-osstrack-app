          ...
          {showForm && (
            <form onSubmit={handleSave}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="turma-categoria" className="text-[10px] text-[var(--text-secondary)] uppercase tracking-wide font-semibold">{tr("categoria")}</label>
                  <select id="turma-categoria" value={categoria} onChange={(e) => setCategoria(e.target.value)}
                ...
                <div className="flex gap-2 flex-wrap">
                  {ICONES.map((ic) => (
                    <button key={ic} type="button" onClick={() => setIcone(ic)}
                      aria-label={`Usar ícone ${ic} na turma`}
                      aria-pressed={icone === ic}
                ...
                <div key={t.id} className="glass-card hover-lift">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(t)} aria-label={`Editar turma ${t.nome}`} title="Editar turma"
                      ...
                      <button onClick={() => handleDelete(t.id)} aria-label={`Excluir turma ${t.nome}`} title="Excluir turma"
                            ...
                            <div className="space-y-1 max-h-40 overflow-y-auto">
                              {alunosNaTurma.map((a) => (
                                <div key={a.id} className="flex items-center justify-between px-3 py-2 rounded-xl bg-[var(--dark-card)]">
                                  ...
                                  <button onClick={() => removerAluno(t.id, a.id)} aria-label={`Remover ${a.nome} da turma`} title="Remover da turma"
                          ...
                          {todosAlunos.length === 0 ? (
                            ...
                            <div className="space-y-1 max-h-40 overflow-y-auto">
                              {todosAlunos.filter((a) => !alunosNaTurma.find((na) => na.id === a.id)).map((a) => (
                                <button type="button" key={a.id} onClick={() => adicionarAluno(t.id, a.id)} aria-label={`Adicionar ${a.nome} à turma`}
