    ...
    <DashboardShell role="aluno">
      <div className="space-y-4">
        <div className="bg-gradient-to-br from-[var(--dark-card)] to-black/40 border border-[var(--dark-border)] rounded-2xl p-5 text-center">
          ...
          <p className="text-xs text-[var(--white-muted)] mt-1">&ldquo;{t("subtitle")}&rdquo;</p>
        </div>
              ...
              return (
                <div key={g.faixa} className="relative pb-7 last:pb-0">
                  ...
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {isCurrent && <span className="badge-gold text-[9px]">{t("voceEstaAqui")}</span>}
                    {isPast && <span className="text-emerald-500 text-xs">✓</span>}
                  </div>
                  <div className="text-[11px] text-[var(--white-muted)] ml-7">{g.graus} {t("graus")} · {g.aulasPorGrau} {t("aulasPorGrau")}</div>
                  {isCurrent && (
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gradient-to-br from-[var(--dark-card)] to-black/40 border border-[var(--dark-border)] rounded-2xl p-5 text-center hover-card">
            <div className="text-sm font-bold uppercase tracking-wide text-[var(--white-muted)] mb-1">{t("grauAtual")}</div>
            <div className="text-3xl font-black gradient-gold-text">{aluno.grau + 1}º</div>
          </div>
          <div className="bg-gradient-to-br from-[var(--dark-card)] to-black/40 border border-[var(--dark-border)] rounded-2xl p-5 text-center hover-card">
            <div className="text-sm font-bold uppercase tracking-wide text-[var(--white-muted)] mb-1">{t("totalAulas")}</div>
